...
{
    struct vscc_label *label_map = assembler->label_map;
    while (label_map) {
        if (label_map->id == id)
...
{
    struct vscc_label *label = vscc_list_alloc((void**)&assembler->label_map, 0, sizeof(struct vscc_label));
...
{
    struct vscc_fill_in_jmp_label *label = vscc_list_alloc((void**)&assembler->fill_in, 0, sizeof(struct vscc_fill_in_jmp_label));

    label->cur_size = vscc_asm_size(assembler);
    label->id = id;
    label->idx = label->cur_size + idx;
...
{
    return vscc_asm_get_label(assembler, id) - ins_length - (!override ? vscc_asm_size(assembler) : override_cur);
}

void vscc_asm_fill_ins(struct vscc_asm_context *assembler, size_t id)
{
    for (struct vscc_fill_in_jmp_label *fill = assembler->fill_in; fill;) {
        if (fill->id == id) {
            int offset = vscc_asm_get_offset(assembler, fill->id, fill->instuction_len, true, fill->cur_size);
            vscc_asm_overwrite(assembler, fill->idx, &offset, sizeof(offset));

            struct vscc_fill_in_jmp_label *next = fill->next;
            vscc_list_free_element((void**)&assembler->fill_in, 0, equals, fill);
            fill = next;
        }
        else {
            fill = fill->next;
